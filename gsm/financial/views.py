...
from gsm.financial.forms import FinanceiroForm, PagamentoFormSet
from gsm.financial.models import Conta
...
def financeirot_list(request):
    ...
    print(request.GET)
    if q:
        contas = Conta.objects.filter(Q(pessoa__name__icontains=q) |
                                      Q(pessoa__id__icontains=q) |
                                      Q(valor_titulo__icontains=q) |
...
def financeirot_client_list(request, pk):
    ...
    if q:
        ...
        #                               Q(valor_titulo__icontains=q, ) |
        #                               Q(historico__descricao__icontains=q))
        contas = Conta.objects.filter(Q(pessoa__id=pk),
                                      Q(valor_titulo__icontains=q, ) |
                                      Q(historico__descricao__icontains=q))
    else:
        contas = Conta.objects.filter(pessoa__is_representative=False,
...
def financeiro_create(request):
    ...
    if request.method == 'POST':
        ...
        context = {'form': FinanceiroForm(initial={'data_vencimento': (date.today() + timedelta(30)).strftime('%d/%m/%Y')})}
