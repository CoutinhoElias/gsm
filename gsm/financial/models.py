...
class Historico(models.Model):
    ...
    def totais(self):
        return Conta.objects.filter(historico_id=self.id).aggregate(Sum('valor_titulo'))[
            'valor_titulo__sum']
...
class Conta(models.Model):
    ...
    pessoa = models.ForeignKey('person.person', on_delete=models.CASCADE, related_name='client_conta')
    historico = models.ForeignKey('Historico', on_delete=models.CASCADE, related_name='historico_conta')
    data_vencimento = models.DateField()
    data_pagamento = models.DateField(null=True, blank=True)
    valor_titulo = models.DecimalField('Valor Titulo', max_digits=15, decimal_places=2)
    operacao = models.CharField(
        max_length=1,
        default='d',
        choices=CONTA_OPERACAO_CHOICES,
